...
class GarminDeviceClient(
    ...
    private fun bindService() {
        ...
        val started = try {
            context.bindService(serviceIntent, Context.BIND_AUTO_CREATE, executor, ciqServiceConnection).also {
                if (it) {
    ...
    private fun sendMessage(msg: Message) {
        ...
        if (service == null) {
            ...
            // Previously this was a silent no-op with no log and no retry, identical
            // to the unhandled bindService() failure that was fixed earlier.
