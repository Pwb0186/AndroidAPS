 ...
 *
 * The V2 push-to-pull machinery (dynamic app discovery, push throttling, the
 * messenger connection watchdog) lives in GarminV2Push - see that file for
...
class GarminPlugin @Inject constructor(
    ...
    /** Dynamic app discovery, push throttling and connection watchdog for the V2
    ...
    private fun onConnectionStateChanged(connected: Boolean) {
        ...
        garminV2Push.isConnected.set(connected)
        if (connected) {
    ...
    /** Swaps in a fresh GarminMessenger and disposes of the old one. Passed into
     *  GarminV2Push.watchdogCheck() as its rebuild callback. */
    private fun rebuildGarminMessenger() {
    ...
    override fun onStart() {
        ...
        garminV2Push.lastMessengerRebuildAt.set(clock.millis())
        disposable.add(
            rxBus
    ...
    private fun sendPhoneAppMessageV2(force: Boolean = false) {
        ...
        if (garminV2Push.watchdogCheck(activeIds) { rebuildGarminMessenger() }) return
